...
import { Plus, Calendar, User, Clock, CheckCircle, ArrowRight } from 'lucide-react';
...
export default function AppointmentPage() {
    ...
    const fetchData = async () => {
        try {
            const [appRes, staffRes, servRes] = await Promise.all([
                api.get(`/appointments?date=${filterDate}${filterStaff ? `&staffId=${filterStaff}` : ''}`),
                api.get('/staff'),
                api.get('/services')
            ]);
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || 'Conflict detected or limit reached');
        }
    };
    ...
    const assignFromQueue = async (staffId: string) => {
        try {
            ...
            toast.error(error.response?.data?.message || 'No eligible appointments in queue');
        }
    };
    ...
    if (loading) return <div className="flex items-center justify-center min-h-screen text-slate-500">Loading Appointments...</div>;
        ...
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500">Manage bookings and the waiting queue.</p>
                </div>
            ...
            <Card className="flex flex-wrap gap-4 items-end p-4">
                <div className="flex-1 min-w-[200px]">
                    <label className="block text-xs font-bold text-slate-400 dark:text-white uppercase mb-1">Filter Date</label>
                ...
                <div className="flex-1 min-w-[200px]">
                    <label className="block text-xs font-bold text-slate-400 dark:text-white uppercase mb-1">Filter Staff</label>
                    <div className="relative">
                        <User size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-white" />
                        <select
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 space-y-4">
                    ...
                    {scheduledApps.length === 0 ? (
                        <Card className="text-center py-12 text-slate-500">No appointments scheduled.</Card>
                    ) : (
                    ...
                    <div className="space-y-4">
                        ...
                        {waitingQueue.length > 0 && (
                            <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
                                <h3 className="text-sm font-bold mb-3 uppercase tracking-wider text-slate-400">Quick Assign</h3>
                                <div className="space-y-2">
                                    {staff.filter(s => s.availabilityStatus === 'Available').map(s => (
                                        <button
                                            key={s._id}
                                        ...
                                        >
                                            <span>Assign earliest to <strong>{s.name}</strong></span>
